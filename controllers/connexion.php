...
public function index() : void{
    ...
    // On instancie le modèle "connexion"
    $this->loadModel('Articles');
    $this->loadModel('Img');
    // On stocke la liste des Contact dans $contact
    $connexions = $this->Articles->getCategorie("connexion");
    foreach($connexions as $key=>$connexion) {
        $connexions[$key]['images']=$this->Img->findByArticleId($connexion['id']);
    }
    // On affiche les données
    // On envoie les données à la vue index
    $this->render('index', compact('connexions')); 
    }
    /**
